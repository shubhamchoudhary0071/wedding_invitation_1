...
const WEDDING_AUDIO_URL = "/assets/audio.mpeg";
...
function getCountdown(d: Date): Countdown {
...
function LanternItem({ delay, size, swayAmp, speed, topOffset }:
  { delay: number; size: number; swayAmp: number; speed: number; topOffset: number }) {
  return (
    <motion.div
    ...
    >
      <img src={lanternImg} alt="" style={{ width: "100%", opacity: 0.88,
        filter: "drop-shadow(0 6px 14px rgba(200,140,40,0.38))" }} />
    </motion.div>
  );
}
...
function LanternColumns() {
  ...
  return (
    <>
      <div className="lantern-col" style={{ ...col, left: 0 }}>
        {LEFT_LANTERNS.map((l, i) => <LanternItem key={i} {...l} />)}
      </div>
      <div className="lantern-col" style={{ ...col, right: 0 }}>
        {RIGHT_LANTERNS.map((l, i) => <LanternItem key={i} {...l} />)}
      </div>
    </>
  );
}
...
/* ─── Reveal ─── */
function Reveal({ children, delay = 0, amount = 0.2, className = "" }:
  { children: ReactNode; delay?: number; amount?: number; className?: string }) {
...
/* ─── Split title ─── */
function SplitTitle({ text, delay = 0, className = "" }:
...
function FloatingPetals() {
  const petals = useMemo(() => Array.from({ length: 10 }, (_, i) => ({
    ...
    delay: Math.random() * 8, dur: 8 + Math.random() * 6,
  })), []);
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
      {petals.map(p => (
        <motion.div key={p.id}
          style={{ position: "absolute", top: "-30px", left: p.x, fontSize: 13, opacity: 0.35 }}
...
/* ══════════════════════════════════════════════════════════
   CURTAIN SCREEN
══════════════════════════════════════════════════════════ */
function CurtainScreen({ onOpen }: { onOpen: () => void }) {
  ...
  const rodStyle: React.CSSProperties = {
    ...
    background: "linear-gradient(180deg,#d4aa50 0%,#a07828 60%,#8b6820 100%)",
    boxShadow: "0 4px 16px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.2)",
    zIndex: 2,
  };

  const rings = [8, 19, 30, 41, 52, 63, 74, 85, 96];

  const makeCurtain = (side: "left" | "right"): React.CSSProperties => ({
    ...
    background:
      side === "left"
        ? "linear-gradient(90deg,#4a0e0e 0%,#7a1a1a 15%,#8f2020 28%,#6b1616 42%,#7f1c1c 55%,#8b2020 68%,#6a1515 82%,#5a1010 100%)"
        : "linear-gradient(90deg,#5a1010 0%,#6a1515 18%,#8b2020 32%,#7f1c1c 45%,#6b1616 58%,#8f2020 72%,#7a1a1a 85%,#4a0e0e 100%)",
    boxShadow:
      side === "left"
        ? "inset -30px 0 60px rgba(0,0,0,0.55), inset -4px 0 8px rgba(0,0,0,0.3), 6px 0 30px rgba(0,0,0,0.5)"
        : "inset 30px 0 60px rgba(0,0,0,0.55), inset 4px 0 8px rgba(0,0,0,0.3), -6px 0 30px rgba(0,0,0,0.5)",
    overflow: "hidden",
    zIndex: 102,
  });
  ...
  return (
    ...
    >
      {/* Left curtain */}
      <motion.div style={makeCurtain("left")}
        animate={state === "opening" ? { x: "-101%" } : { x: 0 }}
        transition={{ duration: 1.7, ease: [0.76, 0, 0.24, 1], delay: 0.08 }}
      >
        {/* Fold lines */}
        <div style={{
          position: "absolute", inset: 0, zIndex: 1,
          background: "repeating-linear-gradient(90deg,transparent 0px,rgba(0,0,0,0.18) 1px,transparent 2px,transparent 28px)",
        }} />
        {/* Sheen */}
        <div style={{
          position: "absolute", inset: 0, zIndex: 1,
          background: "repeating-linear-gradient(90deg,transparent 0px,rgba(255,255,255,0.04) 14px,transparent 28px)",
        }} />
        <div style={rodStyle} />
        {rings.map(p => (
          <div key={p} style={{
            position: "absolute", top: 10, left: `${p}%`,
            width: 11, height: 11, borderRadius: "50%",
            background: "radial-gradient(circle at 35% 35%,#f0cc70,#b08030)",
        ...
        <div style={{
          ...
          background: "linear-gradient(180deg,transparent 0%,#3a0a0a 100%)",
          zIndex: 2,
        }} />
      </motion.div>

      {/* Right curtain */}
      <motion.div style={makeCurtain("right")}
        animate={state === "opening" ? { x: "101%" } : { x: 0 }}
        transition={{ duration: 1.7, ease: [0.76, 0, 0.24, 1], delay: 0.08 }}
      >
        <div style={{
      ...
      {/* Gold rod spanning full width */}
      <div style={{
        position: "absolute", top: 0, left: 0, right: 0, height: 20, zIndex: 104,
        background: "linear-gradient(180deg,#d4aa50 0%,#a07828 60%,#8b6820 100%)",
        ...
        {state === "idle" && (
          <motion.div
            style={{
              ...
              display: "flex", flexDirection: "column", alignItems: "center",
              gap: 28, textAlign: "center", padding: "0 28px",
            }}
          ...
          >
            ...
            >
              ...
              <motion.div
                style={{ fontSize: 38, color: "#e8c87a",
            ...
            {/* Names */}
            <div style={{ color: "#f0dda0" }}>
              <div style={{
                fontSize: "clamp(11px,1.8vw,13px)", letterSpacing: "0.34em",
                textTransform: "uppercase", opacity: 0.65, marginBottom: 18,
              }}>
                you are invited to the wedding of
              </div>
              <div style={{
                fontFamily: "Georgia,serif",
                fontSize: "clamp(48px,9vw,88px)", lineHeight: 0.92,
                letterSpacing: "0.1em", textTransform: "uppercase",
                textShadow: "0 2px 24px rgba(0,0,0,0.7)",
              }}>Rishav</div>
              <div style={{
                fontSize: "clamp(18px,3.5vw,32px)", letterSpacing: "0.26em",
                textTransform: "uppercase", opacity: 0.6, margin: "14px 0",
              }}>Weds</div>
              <div style={{
                fontFamily: "Georgia,serif",
                fontSize: "clamp(48px,9vw,88px)", lineHeight: 0.92,
              ...
              <motion.div
                style={{
                  marginTop: 20,
                  height: 1, background: "linear-gradient(90deg,transparent,rgba(212,170,80,0.6),transparent)",
                }}
              ...
              <div style={{
                marginTop: 14,
                fontSize: "clamp(11px,1.5vw,13px)", letterSpacing: "0.22em",
                textTransform: "uppercase", opacity: 0.55,
              }}>
                20 · 04 · 2026 &nbsp;·&nbsp; Hotel Anntilia Inn
              </div>
            </div>
            ...
            <motion.button
              ...
              style={{
                ...
                padding: "16px 44px",
                borderRadius: "999px",
                background: "linear-gradient(135deg,#c8a060 0%,#e8c87a 50%,#a87828 100%)",
                border: "none", cursor: "pointer",
                fontSize: "clamp(11px,1.5vw,13px)",
                letterSpacing: "0.3em", textTransform: "uppercase",
                color: "#2e1206", fontFamily: "Georgia,serif", fontWeight: 700,
                boxShadow: "0 0 0 1px rgba(200,160,80,0.5), 0 8px 40px rgba(0,0,0,0.6)",
              }}
              whileHover={{
                scale: 1.07,
                boxShadow: "0 0 0 2px rgba(232,200,122,0.8), 0 0 40px rgba(232,200,122,0.25), 0 12px 48px rgba(0,0,0,0.7)",
              }}
              whileTap={{ scale: 0.95 }}
            >
              Open Invitation
            ...
            <motion.div
              style={{
                fontSize: 11, letterSpacing: "0.16em", color: "rgba(240,221,160,0.45)",
                textTransform: "uppercase", display: "flex", alignItems: "center", gap: 10,
              }}
              animate={{ opacity: [0.45, 0.9, 0.45] }}
              transition={{ duration: 2.8, repeat: Infinity }}
            >
              <span>♪</span><span>Music will begin when you open</span><span>♪</span>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
    ...
    >
      ...
      {playing && (
        <motion.span style={{
          ...
          border: "1.5px solid rgba(110,108,65,0.4)", pointerEvents: "none",
        }}
...
export default function WeddingInvite() {
  ...
  const [, setGalleryIndexA] = useState(0);
  const [, setGalleryIndexB] = useState(0);
  ...
  /* Called by curtain — user gesture allows audio play */
  ...
  /* Prevent scroll while curtain is up */
  useEffect(() => {
  ...
  return (
    <div className="page-shell">
      ...
      <AnimatePresence>
        {!curtainOpen && <CurtainScreen onOpen={handleCurtainOpen} />}
      </AnimatePresence>

      {curtainOpen && <MusicButton playing={playing} onToggle={toggleMusic} />}
      ...
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .page-shell{width:100%;min-height:100vh;background:linear-gradient(180deg,#e7e0b9 0%,#efe7c7 28%,#e6d8bb 55%,#d8c7a7 100%);color:#4e5130;overflow-x:hidden;position:relative}
        ...
        .hero{min-height:100svh;display:flex;align-items:center;justify-content:center;background-image:url(${heroWalkway});background-size:cover;background-position:center;color:#f6f0d3;text-align:center;position:relative;overflow:hidden}
        .hero-overlay{position:absolute;inset:0;background:linear-gradient(180deg,rgba(10,10,10,0.1),rgba(10,10,10,0.38))}
        .hero-inner{position:relative;z-index:2;width:min(100%,820px);padding:60px 16px 190px}
        .brand-pill{display:inline-block;margin-bottom:28px;padding:10px 18px;background:rgba(233,224,192,0.25);border:1px solid rgba(255,255,255,0.32);border-radius:999px;font-size:11px;letter-spacing:0.24em;text-transform:uppercase;backdrop-filter:blur(10px)}
        ...
        .floating-car{position:absolute;bottom:24px;left:50%;transform:translateX(-50%);width:min(38vw,480px);min-width:200px;z-index:3;filter:drop-shadow(0 16px 24px rgba(20,18,10,0.3))}

        .invite-panel{background:url(${greenTexture});background-size:cover;background-position:center;text-align:center;color:#576137;min-height:100svh;display:flex;align-items:center}
        .invite-panel-inner{width:min(1100px,calc(100% - 32px));margin:0 auto;display:grid;grid-template-columns:1fr 1fr;gap:48px;align-items:center;padding:60px 0}
        ...
        .divider-line{font-size:18px;margin:4px 0 6px}
        ...
        .invite-text{position:absolute;inset:18% 13% 12%;z-index:3;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}

        .events{background:linear-gradient(180deg,#efe6c7 0%,#eadfbf 100%);min-height:100svh;display:flex;align-items:center}
        .section-title{text-align:center;font-size:clamp(34px,6vw,72px);line-height:0.98;text-transform:lowercase;color:#5a5f36;margin-bottom:12px}
        .section-subtitle{text-align:center;font-size:14px;line-height:1.85;color:#707652;max-width:580px;margin:0 auto 28px}
        .events-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px}
        ...
        .event-content{position:relative;min-height:320px;padding:80px 36px 28px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:#5f6940;z-index:2}
        ...
        .route-btn,.follow-link,.whatsapp-btn{margin-top:16px;display:inline-flex;align-items:center;justify-content:center;min-width:140px;padding:11px 20px;border-radius:999px;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;cursor:pointer}
        ...
        .map-cta{background:url(${pinkTexture});background-size:cover;background-position:center;min-height:70svh;display:flex;align-items:center;text-align:center}
        ...
        .meet{background:url(${greenTexture});background-size:cover;background-position:center;min-height:100svh;display:flex;align-items:center}
        ...
        .couple-frame{position:relative;width:100%;max-width:340px;aspect-ratio:0.72;margin:0 auto 16px;overflow:hidden;border-radius:110px 110px 20px 20px}
        ...
        .rsvp{background:linear-gradient(180deg,#f0e5c8 0%,#ece2c1 100%);min-height:60svh;display:flex;align-items:center;text-align:center}
        .rsvp-card{padding:32px 24px;border-radius:28px;background:rgba(255,255,255,0.3);border:1px solid rgba(110,111,72,0.18);box-shadow:0 12px 28px rgba(72,60,25,0.08);max-width:500px;margin:0 auto}
        ...
        .things{background:url(${greenTexture});background-size:cover;background-position:center;min-height:100svh;display:flex;align-items:center}
        .things-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}
        .thing-card{background:rgba(250,245,226,0.44);border:1px solid rgba(108,117,75,0.2);border-radius:22px;padding:20px 18px;color:#616944;min-height:190px}
        ...
        .follow{background:url(${pinkTexture});background-size:cover;background-position:center;min-height:70svh;display:flex;align-items:center;text-align:center}
        .follow-grid{display:grid;grid-template-columns:1fr 1fr;gap:28px;align-items:center}
        .follow-card{border-radius:26px;padding:28px 20px 22px;background:rgba(255,255,255,0.16);border:1px solid rgba(134,101,112,0.2);max-width:500px;margin:0 auto}
        ...
        .countdown{min-height:100svh;background:linear-gradient(180deg,rgba(7,20,40,0.06),rgba(7,20,40,0.18)),url(${palaceNight});background-size:cover;background-position:center bottom;color:#f0e7ca;text-align:center;display:flex;align-items:center}
      ...
      <div className="invite">
        ...
        <section ref={heroRef} className="section hero">
          ...
          <motion.div style={{
            scale: heroScale, position: "absolute", inset: 0,
            backgroundImage: `url(${heroWalkway})`, backgroundSize: "cover",
          ...
          <div className="container">
            <motion.div className="hero-inner" style={{ y: heroTextY }}>
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.2 }}>
                <div className="brand-pill">Wedding Invitation</div>
              </motion.div>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }}>
                <div className="hero-sub">Save the date</div>
              </motion.div>
              <SplitTitle text="Rishav" className="hero-name" delay={0.6} />
              <motion.div className="hero-weds" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 1.0 }}>Weds</motion.div>
          ...
          <motion.img src={carBlue} alt="Wedding car" className="floating-car" style={{ y: heroCarY }}
          ...
          <div className="invite-panel-inner">
            <Reveal>
              <div className="invite-copy-block">
                ...
                <div className="eyebrow">With the blessings of the divine</div>
                ...
                <div className="invite-copy">
                  Monday, 20th April 2026 · Hotel Anntilia Inn
                </div>
              </div>
            </Reveal>
            <Reveal delay={0.1}>
              <div className="arch-wrap">
                <img src={sageArch} alt="Arch" className="arch-bg" />
                ...
                <div className="invite-text">
                  <div className="eyebrow">With the blessings of the divine</div>
                  <div className="invite-title">Invite</div>
                  <div className="invite-copy">You to join the celebrations of</div>
                  <div className="couple-names">Rishav<br />&amp;<br />Amisha</div>
                </div>
              </div>
            </Reveal>
        ...
        <section className="section map-cta">
          <div className="container">
            <div className="map-grid">
              <Reveal>
                <div>
                  <div className="map-copy"><strong>Hotel Anntilia Inn</strong>Click to open directions</div>
              ...
              <Reveal delay={0.1}>
                <motion.img src={carVintage} alt="Car" className="map-car" animate={{ y: [0, -10, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} />
              </Reveal>
            </div>
          </div>
        </section>
        ...
        <section className="section countdown">
          <div className="container">
            <Reveal>
              <div className="count-title"><SplitTitle text="the countdown begins" delay={0} /></div>
              <div className="count-clock">
                {[
                  ...
                  <motion.div key={u.lbl} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                    <AnimatePresence mode="wait">
                      <motion.div key={u.val} className="count-unit"
              ...
              <div className="count-labels"><span>Days</span><span>Hours</span><span>Minutes</span><span>Seconds</span></div>
              <div className="count-copy">
                Our families are so excited that you are joining Rishav &amp; Amisha in celebrating
                one of the happiest days of their lives. See you on April 20th, 2026 at Hotel Anntilia Inn.
              </div>
              <div className="footer-note">© Missing Piece 2026</div>
            </Reveal>
          </div>
        </section>
